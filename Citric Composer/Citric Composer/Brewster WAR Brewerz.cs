...
{
    public partial class Brewster_WAR_Brewerz : Form
    {
        public Brewster_WAR_Brewerz()
        {
            InitializeComponent();
        }
        ...
        //Variables.
        public SoundWaveArchive file;
        public string filePath = ""; //File path to save.
        public bool fileOpen = false; //If file open.
        public channelPlayer[][] players = new channelPlayer[0][]; //Players.
        int lastIndex = -1;
        bool paused = false;
        SoundFile<ISoundFile> extFile;
        ...
        */
        ...
        public void updateNodes()
        {
            ...
            tree.SelectedNode = tree.Nodes["war"];
            tree.Nodes["war"].ContextMenuStrip = null;

            List<string> expandedNodes = collectExpandedNodes(tree.Nodes);

            foreach (TreeNode e in tree.Nodes["war"].Nodes)
            {
                tree.Nodes["war"].Nodes.RemoveAt(0);
            }
            ...
            if (fileOpen)
            {
                ...
                foreach (var w in file) {
                    ...
                    tree.Nodes["war"].Nodes.Add("Wave " + count, "Wave " + count, 1, 1);
                    tree.Nodes["war"].Nodes[count].ContextMenuStrip = nodeMenu;
                    count += 1;

                }
                tree.Nodes["war"].ContextMenuStrip = rootMenu;
        ...
        private void newToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            stopMusic();
            file = new SoundWaveArchive();
            fileOpen = true;
            filePath = "";
            if (extFile == null) {
                Text = "Brewster's Archive Brewer - New Archive.bfwar";
            } else {
                Text = "Brewster's Archive Brewer - " + extFile.FileName + "." + extFile.FileExtension;
            }
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (warOpen.FileName != "")
            {
                ...
                stopMusic();
                file = new SoundWaveArchive();
                MemoryStream src = new MemoryStream(File.ReadAllBytes(warOpen.FileName));
        ...
        /// <summary>
        /// Open from an internal WAR file.
        /// </summary>
        /// <param name="extFile"></param>
        public void OpenFromInternalWAR(SoundFile<ISoundFile> extFile) {

            this.extFile = extFile;
            file = extFile.File as SoundWaveArchive;
            fileOpen = true;
            this.Text = "Brewster's Archive Brewer - " + extFile.FileName + "." + extFile.FileExtension;
            updateNodes();
        ...
        public void OpenFromFilesystem(string filePath) {

            file = new SoundWaveArchive();
            MemoryStream src = new MemoryStream(File.ReadAllBytes(filePath));
            BinaryDataReader br = new BinaryDataReader(src);
            file.Read(br);
            fileOpen = true;
